...
def _get_cl_gglensing_ordering(probes):
    """
    Utility function to get the indices for Cls from a list of probes
    """
    n_source, n_lens = probes[0].n_tracers, probes[1].n_tracers
    n_tracers=n_source+n_lens
    # Define an ordering for the blocks of the signal vector
    cl_gglensing_index = []
    for s in range(n_source):
        for l in range(n_source,n_tracers):
            cl_gglensing_index.append((s, l))
    return cl_gglensing_index


def find_index(a, b, cl_index):
    if (a, b) in cl_index:
...
def _get_cov_blocks_ordering(probes):
    ...
    for i, j in cl_index:
        for m, n in cl_index:
            cov_blocks.append(
                (find_index(i, m, cl_index), find_index(j, n, cl_index), find_index(i, n, cl_index), find_index(j, m, cl_index))
            )

    return cov_blocks


def _get_cov_gglensing_block_ordering(probes):
    """
    Utility function to get the ordering of the covariance matrix blocks
    """
    n_source, n_lens = probes[0].n_tracers, probes[1].n_tracers
    n_cls = n_source*n_lens
    cl_index = _get_cl_ordering(probes)
    cl_gglensing_index = _get_cl_gglensing_ordering(probes)

    cov_gglensing_block = []
    for i, j in cl_gglensing_index:
        for m, n in cl_gglensing_index:
            cov_gglensing_block.append(
                (find_index(i, m, cl_index), find_index(j, n, cl_index),\
                        find_index(i, n, cl_index), find_index(j, m, cl_index))
            )
    return cov_gglensing_block
...
def angular_cl_gglensing(
    cosmo, ell, probes, transfer_fn=tklib.Eisenstein_Hu, nonlinear_fn=power.halofit
):
    """
    Computes angular Cls for the galaxy-galaxy lensing probe

    Follow the Limber approximation
    ...
    def cl(ell):
        def integrand(a):
            ...
            # Compute and stack the kernels for all probes
            kernels = np.vstack([p.kernel(cosmo, a2z(a), ell) for p in probes])

            # Define an ordering for the blocks of the galaxy-galaxy lensing signal vector
            cl_index = np.array(_get_cl_gglensing_ordering(probes))

            # Extract the galaxy-galaxy lensing kernels
            kernels = lax.map(lambda inds: kernels[inds[0]] * kernels[inds[1]], cl_index)
...
def gaussian_cl_gglensing_covariance(ell, probes, cl_signal, cl_noise, f_sky=0.25, sparse=True):
    """
    Computes a Gaussian covariance for the angular cls of the galaxy-galaxy lensing probe
    ...
    # Retrieve ordering for blocks of the covariance matrix
    cov_gglensing_block = np.array(_get_cov_gglensing_block_ordering(probes))
    def get_cov_gglensing_block(inds):
        a, b, c, d = inds
        cov = (cl_obs[a] * cl_obs[b] + cl_obs[c] * cl_obs[d]) / norm
        return cov * one
    ...
    # We could compress this further using the symmetry of the blocks, but
    # it is easier to invert this matrix with this redundancy included.
    cov_mat = lax.map(get_cov_gglensing_block, cov_gglensing_block)
